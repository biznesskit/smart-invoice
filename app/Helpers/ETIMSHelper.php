...
class ETIMSHelper
{
    ...
    public static function insertStockInputOut(StockInOut $stockInOut, Branch $branch, User $staff)
    {
        ...
        $data = [
            ...
            "totTaxblAmt" => round($stockInOut->total_taxable_amount, 4),
            "totTaxAmt" => round($stockInOut->total_tax_amount, 4),
            "totAmt" => round($stockInOut->total_amount, 4),
    ...
    public static function recordNewSale(Invoice $invoice, Branch $branch, User $staff)
    {
        ...
        $data = [
            ...
            "taxblAmtA" => round($invoice->taxable_amount_A, 4),
            "taxblAmtB" => round($invoice->taxable_amount_B, 4),
            "taxblAmtC" => round($invoice->taxable_amount_C, 4),
            "taxblAmtD" => round($invoice->taxable_amount_D, 4),
            "taxblAmtE" => round($invoice->taxable_amount_E, 4),
            ...
            "taxAmtA" => round($invoice->tax_amount_A, 4),
            "taxAmtB" => round($invoice->tax_amount_B, 4),
            "taxAmtC" => round($invoice->tax_amount_C, 4),
            "taxAmtD" => round($invoice->tax_amount_D, 4),
            "taxAmtE" => round($invoice->tax_amount_E, 4),
            "totTaxblAmt" => round($invoice->total_taxable_amount, 4),
            "totTaxAmt" => round($invoice->total_tax_amount, 4),
            "totAmt" => round($invoice->total_amount, 4),
    ...
    private static function generateStockInputOutputItemsList(Object $items)
    {
        ...
        foreach ($items as $item) :
            $arr = [
                ...
                "taxTyCd" => $item->tax_type_code,
                "taxblAmt" => round($item->taxable_amount, 4),
                "taxAmt" => round($item->tax_amount, 4),
                "totAmt" => round($item->total_amount, 4),
