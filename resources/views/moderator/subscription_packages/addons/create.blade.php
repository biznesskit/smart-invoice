...
<h2 class="mt-2">Create addon</h2>

<form id="addon-form" action="{{route('moderator.subscription.packages.addons.store') }}" onsubmit="confirm_submit(event,'addon-form')" method="post" class="bg-light edit_form">
    @csrf
  <div class="row">
    ...
    <div class="form-group col-md-6 mb-2">
      ...
      <input type="number" name="validity" min="1" class="form-control" placeholder="Enter digits"  value="{{isset($addon) && is_null(old('validityInDays')) ? $addon->validityInDays : old('validityInDays') }}" required >
    </div>
  </div>
  ...
  <div class="row">
    <div class="form-group col-md-12 mb-2">
      ...
      <textarea name="description" type="number" class="form-control"  placeholder="">{{isset($addon) && is_null(old('description')) ? $addon->description : old('description') }}</textarea>
    </div>
  </div>
